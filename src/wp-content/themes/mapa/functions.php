...
//Adiciona os arquivos de CustomPostType para Esporos
require_once (get_stylesheet_directory() . '/esporos/custom-esporos.php');

//Retira o Post do Menu
...
function remove_admin_menu_item () {
	...
	global $menu;
	foreach($menu as $k => $v) {
			if (!in_array ( $k, array (2,4,6,59)))
			    unset ($menu [$k]);
	}
}

// Permite 1 post por usuário
add_action("save_post", "only_1_post");
function only_1_post( $post ) {
    ...
    $sql = $wpdb->prepare("
        ...
        WHERE 1=1
            ...
            AND post_type = 'esporos'
            AND post_author = '" . $user->ID . "'
        LIMIT 1
    ");
    $created_post = $wpdb->get_var( $sql );
    if ( $created_post && $created_post != $post )
        wp_die( 'Você só pode criar <a href="' . admin_url() . 'post.php?action=edit&post=' . $created_post . '">um ponto no mapa</a> com a sua conta.' );
